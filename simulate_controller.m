function log = simulate_controller(controllerFcn, ctrlName, env, sim)
%SIMULATE_CONTROLLER Kinematic sim + raycast sensing + richer logs (3D/vertical)
%
% controllerFcn: @(t, state, ranges, goal, env, sim, mem) -> [cmd, mem]
% cmd: struct('vx','vy','vz','wz') in BODY frame
%
% Logs added:
%   log.rmin, log.speed, log.mode, log.pF (virtual "front" range used for progress)
% Fix:
%   collision check uses newState.
...
ranges = raycast_ranges_obbs(state, env.obbs, env.max_range);
...
log.rF  = zeros(N,1); log.rB = zeros(N,1); log.rL = zeros(N,1); log.rR  = zeros(N,1);
log.rU  = zeros(N,1); log.rD = zeros(N,1);
...
% Mirrors the real onboard mission's own exit criterion (see
% app_cf21_slam_tunnel.c: openCount>=3-of-4 held for exitHoldS) so the
% sim can stop the way the real vehicle would land, instead of
% continuing to wander/loop in open space once clear of the tunnel.
% Opt-in and defaults-preserving: existing callers (run_compare_*,
% run_dropout_trials, etc.) are unaffected unless they explicitly set
% sim.exit_detect.
...
if collision_sphere_obbs(state, sim.robot_radius, env.obbs)
    ...
    log.rF(1)=ranges.rF; log.rB(1)=ranges.rB; log.rL(1)=ranges.rL; log.rR(1)=ranges.rR;
    log.rU(1)=ranges.rU; log.rD(1)=ranges.rD;
    log.rmin(1)=min([ranges.rF,ranges.rB,ranges.rL,ranges.rR,ranges.rU,ranges.rD]);
    log.speed(1)=0;
    [mode, pF] = infer_mode_and_pF(state, goal, sim, ranges);
    log.mode(1)=mode;
    log.pF(1)=pF;
    log.N = 1;
    log.tlog = 1;
...
for k = 1:N
    ...
    log.rF(k)=ranges.rF; log.rB(k)=ranges.rB; log.rL(k)=ranges.rL; log.rR(k)=ranges.rR;
    log.rU(k)=ranges.rU; log.rD(k)=ranges.rD;

    log.rmin(k)  = min([ranges.rF,ranges.rB,ranges.rL,ranges.rR,ranges.rU,ranges.rD]);
    ...
    % exit-then-stop check (opt-in, see sim.exit_detect above) -- uses
    % the SAME (possibly noisy/dropout-held) ranges the controller just
    % saw, since the real onboard exit detection is subject to the same
    % sensing imperfections, not ground truth.
    if doExitDetect
        frontOpen = isfinite(ranges.rF) && (ranges.rF > ed.front_m);
    ...
    % collision check (FIX: check newState)
...
function [mode, pF] = infer_mode_and_pF(state, goal, sim, ranges)
% Mirror the util_nav_mode + util_virtual_ranges logic WITHOUT dependencies.
